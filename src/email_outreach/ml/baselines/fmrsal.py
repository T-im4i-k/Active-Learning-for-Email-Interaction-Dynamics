...
class FMRSALConfig(AbstractConfig):
    ...
    def to_filename(self) -> str:
        return (
            f"fm_rsal_explore_k={self.latent_dim}"
...
class _FM(nn.Module):
    def __init__(self, n_features: int, k: int):
        ...
        nn.init.normal_(self.v.weight, 0.0, 1e-2)

    def forward(self, x: torch.LongTensor) -> torch.Tensor:
        lin = self.linear(x).sum(dim=1)  # (B,1)
        Vx = self.v(x)  # (B, F, k)
        interactions = 0.5 * ((Vx.sum(1) ** 2 - (Vx**2).sum(1))).sum(1)  # (B,)
...
class FMRSALModel(AbstractContextualModel):
    ...
    def fit(self, train: AutoencoderDataset, val: AutoencoderDataset) -> None:
        ...
        self.user2idx = {
            u: i for i, u in enumerate(sorted(train.mails["user_id"].unique()))
        }
        offset = len(self.user2idx)
        self.mail2idx = {
            m: i + offset
            for i, m in enumerate(sorted(train.mails["mailshot_id"].unique()))
        }
        self.n_features = offset + len(self.mail2idx) + 1  # +1 for <UNK>

        self.unk_mail_idx = self.n_features - 1  # last slot

        self._historic_X, self._historic_y = self._df_to_tensor(
            train.mails, extend_vocab=False
        )
    ...
    def predict(self, data: AutoencoderDataset) -> List[DefaultMetrics]:
        ...
        for mail_id in df_val["mailshot_id"].unique():
            ...
            df_explore["time_to_open"] = pd.to_timedelta(
                df_explore["time_to_open"], unit="h"
            )
            df_explore["original_opened"] = df_explore["opened"].copy()
            within = (df_explore["opened"] == 1) & (
                df_explore["time_to_open"] <= pd.Timedelta(hours=self.cfg.wait_hours)
            )
            ...
            X_target, y_target = self._df_to_tensor(
                df_target, extend_vocab=False, return_y=True
            )

            # 4) RSAL loop -------------------------------------------- #
            for r in range(self.cfg.repetitions):
                logger.info(
                    f"[FM-RSAL-EXP] mail={mail_id} " f"rep {r+1}/{self.cfg.repetitions}"
                )

                # bootstrap
                n = X_train.size(0)
                size = int(self.cfg.subsample_ratio * n)
                idx = torch.randint(high=n, size=(size,), device=DEVICE)
                X_boot = X_train[idx]
                y_boot = y_train[idx]
                ...
                # log metrics
                for mail_id, opened in zip(
                    df_explore["mailshot_id"].values,
                    df_explore["original_opened"].values,
                ):
                    metrics.append(
                        DefaultMetrics(
                ...
                for opened, prob, uid in zip(
                    y_target, probs, df_target["user_id"].values
                ):
                    metrics.append(
                        DefaultMetrics(
                            mailshot_id=int(mail_id),
    ...
    def _df_to_tensor(
        self, df: pd.DataFrame, extend_vocab: bool = False, return_y: bool = False
    ) -> Tuple[torch.LongTensor, torch.FloatTensor | np.ndarray]:

        def _map_user(u):
            return self.user2idx.get(
                u,
                self.user2idx.setdefault(u, len(self.user2idx)) if extend_vocab else 0,
            )
        ...
        X = torch.tensor(
            np.stack(
                [
                    df["user_id"].map(_map_user).values,
                    df["mailshot_id"].map(_map_mail).values,
                ],
                axis=1,
            ),
            dtype=torch.long,
