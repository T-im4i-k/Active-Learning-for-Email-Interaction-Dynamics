...
class FMFCDBModel(AbstractContextualModel):
    ...
    def fit(self, train: AutoencoderDataset, val: AutoencoderDataset) -> None:
        ...
        self.user_features, self.user_stats = self._extract_user_features_vectorized(
            train
        )
    ...
    def _extract_user_features_vectorized(
        self, data: AutoencoderDataset
        ...
        features[:, 0] = torch.tensor(
            open_rates.reindex(range(num_users), fill_value=0).values,
            dtype=torch.float32,
        )
        ...
        features[:, 1] = torch.tensor(
            activity_levels.reindex(range(num_users), fill_value=0).values,
            dtype=torch.float32,
        )
        ...
        def recent_open_rate(group):
            return (
                group.tail(5)["opened"].mean()
                if len(group) >= 5
                else group["opened"].mean()
            )

        recent_rates = user_groups.apply(recent_open_rate)
        features[:, 2] = torch.tensor(
            recent_rates.reindex(range(num_users), fill_value=0).values,
            dtype=torch.float32,
        )
        ...
        features[:, 3] = torch.tensor(
            consistency.reindex(range(num_users), fill_value=0.5).values,
            dtype=torch.float32,
        )
        ...
        if self.config.feature_dim > 4:
            ...
            features[:, 4] = torch.tensor(
                trends.reindex(range(num_users), fill_value=0.5).values,
                dtype=torch.float32,
            )
        ...
        if self.config.feature_dim > 5:
            ...
            features[:, 5] = torch.tensor(
                decay.reindex(range(num_users), fill_value=0.5).values,
                dtype=torch.float32,
            )
        ...
        logger.info(
            f"Features extracted for {num_users} users with {self.config.feature_dim} dimensions"
        )
    ...
    def _learn_weights_sgd(self, train: AutoencoderDataset):
        ...
        for epoch in range(self.config.n_epochs):
            # Sample mailshots
            sampled_mailshots = np.random.choice(
                n_mailshots, sample_size, replace=False
            )
            ...
            for mailshot_idx in sampled_mailshots:
                ...
                # Filter valid users
                valid_users = [
                    u for u in user_indices if u < self.user_features.shape[0]
                ]
                ...
                # Update weights
                self.feature_weights += self.config.learning_rate * gradients.mean(
                    dim=0
                )
            ...
            if epoch % 2 == 0:
                ...
                logger.info(
                    f"Epoch {epoch}, Avg Loss: {avg_loss:.4f}, Samples: {n_samples}"
                )

    def predict(
        self, data: AutoencoderDataset, use_batched_active_learning: bool = True
    ) -> List[DefaultMetrics]:
    ...
    def _predict_standard(self, data: AutoencoderDataset) -> List[DefaultMetrics]:
        ...
        explore_decisions = (
            np.random.random(n_predictions) < self.config.exploration_rate
        )
        pred_idx = 0
        ...
        for i in range(len(val)):
            ...
            for _, mail in mailshot_mails.iterrows():
                ...
                if user_id < self.user_features.shape[0]:
                    ...
                    model_prediction = torch.sigmoid(
                        torch.dot(self.feature_weights, user_feat)
                    ).item()
    ...
    def predict_batched_active_learning(
        self, data: AutoencoderDataset
    ) -> Tuple[List[DefaultMetrics], Dict[int, List[Tuple[int, float]]]]:
        """
        ...
        logger.info(
            f"Starting batched prediction for {len(all_users)} users in {self.config.num_batches} batches"
        )
        ...
        for stage in range(self.config.num_batches):
            ...
            logger.info(
                f"Batch {stage + 1}/{self.config.num_batches}: Predicting for {len(batch_users)} users"
            )
            ...
            for user_id in batch_users:
                ...
                if user_id < self.user_features.shape[0]:
                    ...
                    if (
                        stage < early_batch_threshold
                        and np.random.random()
                        < self.config.exploration_rate
                        * self.config.early_exploration_boost
                    ):
                        prediction = np.random.random()
            ...
            if stage > 0 and stage % self.config.update_frequency == 0:
                self._update_features_from_observations(
                    sent_at_stage, observed_opens, stage
                )

            logger.info(
                f"Batch {stage + 1}: Sent to {len(batch_users)} users, "
        ...
        logger.info(
            f"Completed batched prediction. Total opens: {len(observed_opens)}/{len(all_users)} "
            f"({100.0 * len(observed_opens) / len(all_users):.1f}%)"
        )

        return all_metrics, batch_scores

    def _update_opens_all_stages(
        self,
        ...
        for stage, users in grouped_sent_at_stage.items():
            ...
            stage_opened = dataset.select_opened_indices(
                mailshot_id, users, time_window
            )
            opened_indices.extend(stage_opened)

        return list(set(opened_indices))  # Remove duplicates

    def _update_features_from_observations(
        self,
        sent_at_stage: Dict[int, int],
        ...
        for user_id in sent_at_stage:
            if user_id < self.user_features.shape[0]:
                ...
                self.user_features[user_id, 0] = (
                    1 - update_weight
                ) * old_val + update_weight * float(opened)
                ...
                self.user_features[user_id, 2] = (
                    1 - update_weight * 1.5
                ) * old_recent + update_weight * 1.5 * float(opened)
                ...
                if self.config.feature_dim > 3:
                    ...
                    self.user_features[user_id, 3] = (
                        1 - update_weight * 0.5
                    ) * old_consistency + update_weight * 0.5 * consistency_update
