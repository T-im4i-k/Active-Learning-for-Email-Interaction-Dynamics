    ...
    def predict(self, data: AutoencoderDataset) -> List[DefaultMetrics]:
        ...
        for i in range(self.config.repetitions):
            ...
            predictions = torch.concatenate(
                [samples[val.mailshot_user_indices(i)] for i in val.mailshot_ids]
            )
            embeddings = torch.concatenate(
                [
                    torch.tensor(val._mailshot_embeddings[i])[
                        val.mailshot_user_indices(mailshot_id)
                    ]
                    for i, mailshot_id in enumerate(val.mailshot_ids)
                ]
            )
