...
class FMPSALModel(AbstractContextualModel):
    ...
    def fit(self, train: AutoencoderDataset, val: AutoencoderDataset) -> None:
        ...
        self.train_user_popularity = train.mails[train.mails["opened"] == 1][
            "user_id"
        ].value_counts()
    ...
    def predict(self, data: AutoencoderDataset) -> List[DefaultMetrics]:
        ...
        for mail_id in df_val["mailshot_id"].unique():
            ...
            for r in range(self.cfg.repetitions):
                logger.info(
                    f"[FM-PSAL] mail={mail_id} rep={r+1}/{self.cfg.repetitions}"
                )

                n = X_train.size(0)
                idx = torch.randint(
                    high=n, size=(int(self.cfg.subsample_ratio * n),), device=DEVICE
                )
                X_boot = X_train[idx]
                y_boot = y_train[idx]
