...
class ContextualBanditMetrics(AbstractMetrics):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "user_cluster": (
                int(self.user_cluster)
                if self.user_cluster is not None and hasattr(self.user_cluster, "item")
                else self.user_cluster
            ),
    ...
    def from_lists(
        ...
        return [
            ...
            for p, o, pred, cluster in zip(
                present_in_prediction, opened, prediction, user_clusters
            )
        ]
...
class AbstractContextualModel(ABC):
    ...
    def grid_search(
        ...
        """
        logger.info(
            f"Starting grid search for {cls.model_name()} with {len(param_grid)} parameters"
        )
        ...
        logger.info(
            f"Dataset loaded. Train size: {len(train.mails)}, Val size: {len(val.mails)}"
        )
        ...
        if n_samples is None:
            ...
            grid = list(
                ParameterSampler(
                    param_grid, n_iter=n_samples, random_state=random_state
                )
            )
    ...
    def _save_grid_search_results(
        ...
        for rank, result in enumerate(top_results, 1):
            ...
            logger.info(
                f"#{rank:02d} Score={result['score']:.4f} {recall_str} "
